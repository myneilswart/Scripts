...
Results are:
  ...
  • Saved as a clickable HTML report and/or txt file to selected location (will save to Desktop by default)
  • Saved to a newly created folder if folder entered does not already exist. Working on a fix to create new
    files for results instead of over-writing old ones.
  • Over-written if files are already saved from previous scans in same folder. Working on a fix. Recommend   
    moving scan results to a different folder before proceeding with new scan for now.
...
Hidden file support:
  ...
  • System directories always skipped regardless of hidden setting:
    ...
    "program files (x86)", "programdata", "recovery", "boot", "efi". Working on possible fix. Recommend 
    checking manually for now.

Requirements: Python 3.10+  (no third-party libraries needed)

Usage:
    python file_finder.py or double-click .py file
"""
...
FILE_ATTRIBUTE_REPARSE_POINT = 0x400   # symlinks AND junctions/mount points
...
def is_reparse_point(path: Path) -> bool:
    """
    True if path has the Windows REPARSE_POINT attribute — this covers
    both symlinks and NTFS junctions/mount points. pathlib's is_symlink()
    does NOT reliably detect junctions, so this is checked separately
    before descending into a directory during the walk (see keep_dir in
    scan()) to avoid infinite loops on self-referencing junctions such as
    the classic "C:\\Documents and Settings" -> "C:\\Users" link or
...
def get_desktop() -> Path:
    try:
        ...
        winreg.CloseKey(key)
        # The registry value is sometimes stored as an unexpanded string
        # (e.g. "%USERPROFILE%\Desktop"). If we don't expand it, mkdir()
        # later creates a literally-named "%USERPROFILE%" folder in the
        # current working directory.
        desktop = os.path.expandvars(desktop)
        path = Path(desktop)
        return path if path.exists() else Path.home() / "Desktop"
    except Exception:
...
def scan(root: Path, extensions: set[str], scan_mode: str) -> list[FileResult]:
    ...
    for dirpath, dirnames, filenames in os.walk(root, topdown=True, onerror=None):
        ...
        def keep_dir(name: str) -> bool:
            p = current / name
            # Always refuse to descend into junctions / symlinked folders.
            # Windows is full of these (Documents and Settings, AppData's
            # "Application Data" alias, OneDrive mirrors, etc.) and several
            # of them point back at an ancestor directory. os.walk has no
            # built-in protection against that for junctions (only for true
            # symlinks via followlinks=False), so without this check a
            # full C:\ scan can loop forever and never reach the point
            # where results are written out.
...
def pick_category() -> list[str]:
    ...
    header("What type of media are you trying to find?")
...
def pick_scan_mode() -> str:
    header("Would you like to find hidden/system files as well?")
...
def pick_output_format() -> str:
    ...
    header("How would you like to save the output?")
    options = [
...
def pick_output_path(label: str) -> Path:
    header("Where would you like to save results?")
    desktop = get_desktop()
    print(f"  Default save folder:")
    print(f"  {desktop}")
    print()
    raw = input("  Press Enter to use Desktop, or enter full folder path › ").strip()
...
def confirm(
    ...
    print()
    return prompt_yes_no("Ready to roll?", default_yes=True)
...
def main():
    ...
    while True:
        ...
        if confirm(label, fmt_labels, scan_mode, output_fmt, output_base, ROOT):
...
if __name__ == "__main__":
    try:
        ...
        # If the script is launched by double-clicking the .py file,
        # an unhandled exception would otherwise close the console
        # window instantly — making a real error look identical to
        # "the script ran and silently produced nothing".
        import traceback
        print("\n  ❌  File Finder crashed with an error:\n")
        traceback.print_exc()
